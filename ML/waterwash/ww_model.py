...
tf.app.flags.DEFINE_integer('training_iteration', 2100000,
...
def main(_):
    ...
    # tf Graph Input
    X = tf.placeholder(tf.float32, shape = [None, num_inputs],name="first_placeholder")
    Y = tf.placeholder(tf.float32, shape = [None, num_classes],name="second_placeholder")
    # Set model weights
    W = tf.cast(tf.Variable(rng.randn(num_inputs, num_classes), name="weight"), tf.float32)
    ...
    writer = tf.summary.FileWriter("~/Desktop/ML/waterwash/model_1") # Path to save tensorboard file
